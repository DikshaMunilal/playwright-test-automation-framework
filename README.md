...
|---|---|
| `npm test` | Run everything (API + UI, parallel) |
| `npm run test:api` | API suite only |
| `npm run test:ui` | UI suite only |
| `npm run test:headed` | UI suite with visible browser |
| `npm run report` | Open the Playwright HTML report |

Monocart report: open `monocart-report/index.html` after any run.
...
**Local execution** — full suite (10 API + 15 UI tests) using parallel workers:

![Local run — 25 passed](docs/evidence/local-run.png)
